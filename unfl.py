...
class Cleaner:
    ...
    def unfollow(self, idss, User):
        ...
        print(f' ~ [{self.del_count}] </> Unfollow Success [{User}] </> ID [{idss}] ')
    ...
    def get_following(self):
        ...
        while True:
            try:
                ...
                get=info.json()["followings"]
                
                
                for x in get:
                    idss = x['uid']
                    User = x['unique_id']
                unfl= threading.Thread(target=self.unfollow, args=(idss, User,))
                unfl.start()
            except:
               print(' ~ Lỗi ')
               pass
...
if __name__ == '__main__':
    ...
    cktt = input('Enter Cookie : ')
    print('- '*25)
    sessid=cktt.split('sessionid=')[1].split(';')[0]
    start(sessid)
